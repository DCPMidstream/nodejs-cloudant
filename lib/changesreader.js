...
class ChangesReader {
  ...
  start(opts) {
    ...
    async.doWhilst(function(next) {
      ...
      // formulate changes feed longpoll request

      // calculate this batchsize. Usually it's 'batchsize', but if
      // we have maxChanges limit, we don't want to over-fetch too 
      // many changes
      ...
      self.request(req, function(err, data) {
        ...
        if (!err && data) {
          ...
          // error (wrong password, bad since value, service down etc)
    ...
    function() {
      // continue if we're in indefinte mode or we haven't met or changes quote yet
